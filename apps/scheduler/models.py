...
class Viaje(models.Model):
    trip_id = models.CharField(max_length=100, blank=True, null=True, help_text="ID interno de viaje GTFS")
    ...
    shape = models.ForeignKey(
        ...
        blank=True, 
        related_name="viajes",
        help_text="Trazado geoespacial asociado a este viaje"
    )
    direction_id = models.IntegerField(
        default=0, 
        choices=[(0, "Ida"), (1, "Vuelta")],
        help_text="0 para Ida, 1 para Vuelta"
    )

    def __str__(self):
        dir_label = "Ida" if self.direction_id == 0 else "Vuelta"
        return f"Viaje #{self.id} - {self.route.route_short_name} ({dir_label})"
...
class TiempoParada(models.Model):
    ...
    def __str__(self):
        return f"Viaje #{self.trip_id} - Sec {self.stop_sequence}: {self.stop.stop_name}"